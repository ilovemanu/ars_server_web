package com.gompeisquad.parser.system;
...
import com.gompeisquad.parser.utils.TimeConverter;
    ...
    public void depTimeFlightDFS(ArrayList<ArrayList<Flight>> res, ArrayList<Flight> subres,
        ...
        for (Flight f : resFlights) {
            if (isSeatAvailable(f, seatClass)) {
                ...
                // time validation
                if (!isValidTime(f) || (subres.size() == 0 && !f.departureLocalTime().toLocalDate().isEqual(this.depDate)))
                    continue;
    ...
    public void arrTimeFlightDFS(ArrayList<ArrayList<Flight>> res, ArrayList<Flight> subres,
        ...
        for (Flight f : resFlights) {
            if (isSeatAvailable(f, seatClass)) {
                ...
                if (!isValidTime(f) || (subres.size() == 0 && !f.arrivalLocalTime().toLocalDate().isEqual(this.arrDate)))
                    continue;
                subres.add(f);
